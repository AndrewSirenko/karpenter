...
var _ = Describe("Balanced Scoring", func() {
	...
	Describe("ScoreMove (SavingsRatio)", func() {
		...
		// Pre-fix: savings=0.50-0.10(spot)=0.40 -> score=1.00 APPROVED.
		// Post-fix: savings=0.50-0.40(on-demand)=0.10 -> score=0.25 REJECTED.
		It("should REJECT a move when EstimatedSavings excludes spot offerings for an on-demand-only NodePool", func() {
			...
			sourceIT := &cloudprovider.InstanceType{
				...
				Offerings: cloudprovider.Offerings{
					{Price: 0.50, Available: true, Requirements: scheduling.NewLabelRequirements(map[string]string{
						v1.CapacityTypeLabelKey:  v1.CapacityTypeOnDemand,
						corev1.LabelTopologyZone: "test-zone-1",
					})},
				},
			...
			destIT := &cloudprovider.InstanceType{
				Name: "dest-type",
				Offerings: cloudprovider.Offerings{
					{Price: 0.10, Available: true, Requirements: scheduling.NewLabelRequirements(map[string]string{
						v1.CapacityTypeLabelKey:  v1.CapacityTypeSpot,
						corev1.LabelTopologyZone: "test-zone-1",
					})},
					{Price: 0.40, Available: true, Requirements: scheduling.NewLabelRequirements(map[string]string{
						v1.CapacityTypeLabelKey:  v1.CapacityTypeOnDemand,
						corev1.LabelTopologyZone: "test-zone-1",
					})},
				},
